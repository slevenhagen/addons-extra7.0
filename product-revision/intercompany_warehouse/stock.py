...
class stock_move(orm.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context is None:
            context = {}
        """ This is a tricked test to make the context work
        when return the picking """
        if vals and vals.get('picking_id'):
            picking_id = vals.get('picking_id')
            picking_obj = self.pool.get('stock.picking')
            translated_name = _('%s-%s-return') % ('newname', 'oldname')
            if len(translated_name.split('-')) == 3:
                translated = translated_name.split('-')[2]
            name = picking_obj.read(cr, uid, picking_id,
                                    ['name'], context=context)['name']
            if name and name.endswith(translated):
                context['do_not_reverse'] = True
        """ This part should work if the context was well transmitted 
        in the native create_returns action (stock.return.picking) 
        Please do not remove this commented part. """
        if not context.get('do_not_reverse'):
