...
class mrp_production(orm.Model):
    ...
    def _get_default_value_to_copy(self, cr, uid, production,
                                   quantity_to_do, context=None):
        if context is None:
            context = {}
        return {
        ...
        for production in self.browse(cr, uid, ids, context=context):
            ...
            if production.state == 'partially_ready' and \
                ...
                if quantity_to_do > 0:
                    ...
                    if not new_prod_id:
                        default_vals = self.\
                            _get_default_value_to_copy(cr, uid, production,
                                                       quantity_to_do,
                                                       context=context)
                    ...
                    qty_change_obj.\
                        change_prod_qty(cr, SUPERUSER_ID, [wiz_id],
        ...
        if not vals.get('state'):
            for prod in self.browse(cr, uid, ids, context=context):
                if prod.state == 'confirmed' and prod.product_qty_ready:
                    ...
                    self.write(cr, uid, [prod.id],
                               {'state': 'confirmed'}, context=context)
                elif prod.state == 'partially_ready' and prod.product_qty_ready >= prod.product_qty:
                    self.write(cr, uid, [prod.id],
