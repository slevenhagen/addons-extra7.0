        ...
        for move in self.browse(cr, uid, ids, context=context):
            if not move.prodlot_id and \
               ...
               ( \
                   ...
                   (move.product_id.track_internal)
               )):
                return False
        return True
...
class stock_picking_in(orm.Model):
    ...
    def _check_single_serial(self, cr, uid, ids, context=None):
        for record in self.browse(cr, uid, ids, context=context):
            prod = {}
            prds = []
            ...
            for move in record.move_lines:
                if not (move.prodlot_id and move.product_id.lot_split_type == 'single'):
                    continue
                if not prod.get((move.product_id.id, move.prodlot_id.id),False):
                    prod[(move.product_id.id, move.prodlot_id.id)] = 0
                prod[(move.product_id.id, move.prodlot_id.id)] += 1
                
            for k,v in prod.iteritems():
                if v>1:
                    pname = self.pool.get('product.product').browse(cr, uid, k[0]).name
                    sname = self.pool.get('stock.production.lot').browse(cr, uid, k[1]).name
                    raise orm.except_orm(_('Error'), _('You cannot use serial no. %s for product %s more than once in move lines.')% (sname, pname))
        return True

    _constraints = [
        (_check_single_serial, 'You cannot choose same serial no. for product type single.',
            ['prodlot_id','product_id','picking_id']),
       ]
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            for k,v in prod.iteritems():
                if v>1:
                    pname = self.pool.get('product.product').browse(cr, uid, k[0]).name
                    sname = self.pool.get('stock.production.lot').browse(cr, uid, k[1]).name
                    raise osv.except_osv(_('Error'), _('You cannot use serial no. %s for product %s more than once in move lines.')% (sname, pname))
        return True
...
class stock_production_lot(orm.Model):
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if 'location_id' in context:
            ...
            location_id = context['location_id'] and [context['location_id']] or [] 
            cr.execute(
                   ''' SELECT 
                      stock_report_prodlots.prodlot_id
                    FROM 
                      public.stock_report_prodlots
                    WHERE 
                      stock_report_prodlots.qty > 0 AND 
                      stock_report_prodlots.location_id = %s AND
                      stock_report_prodlots.prodlot_id IS NOT NULL''',
                      (tuple(location_id)))
            results = cr.fetchall()
...
class stock_production_lot(orm.Model):
    ...
    _sql_constraints = [
        ('name_ref_uniq_NPG', 'unique (name, product_id)', 'The combination of Serial Number and Product must be unique !'),
    ]
